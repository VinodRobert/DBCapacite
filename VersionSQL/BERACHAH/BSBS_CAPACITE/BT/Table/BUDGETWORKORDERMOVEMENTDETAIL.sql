/****** Object:  Table [BT].[BUDGETWORKORDERMOVEMENTDETAIL]    Committed by VersionSQL https://www.versionsql.com ******/

SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
CREATE TABLE [BT].[BUDGETWORKORDERMOVEMENTDETAIL](
	[DETAILID] [bigint] IDENTITY(1,1) NOT NULL,
	[PROJECTCODE] [int] NULL,
	[TOOLCODE] [varchar](35) NULL,
	[WWWITEMCODE] [bigint] NULL,
	[REQID] [int] NULL,
	[REQDATE] [datetime] NULL,
	[REQNUMBER] [varchar](25) NULL,
	[LINENUMBER] [int] NULL,
	[PRQTY] [decimal](18, 4) NULL,
	[PRUOM] [varchar](15) NULL,
	[PRRATE] [decimal](18, 4) NULL,
	[PRAMOUNT] [decimal](18, 2) NULL,
	[PRSTATUS] [int] NULL,
	[OPENPR] [decimal](18, 4) NULL,
	[CANCELLEDQTY] [decimal](18, 4) NULL,
	[REJECTQTY] [decimal](18, 4) NULL,
	[ORDID] [int] NULL,
	[ORDNUMBER] [varchar](25) NULL,
	[ORDDATE] [datetime] NULL,
	[RECONID] [int] NULL,
	[BILLINGDETAILID] [int] NULL,
	[ORDQTY] [decimal](18, 4) NULL,
	[ORDRATE] [decimal](18, 4) NULL,
	[ORDAMOUNT] [decimal](18, 4) NULL,
	[ORDSTATUS] [int] NULL,
	[PRPOQTYDIFF] [decimal](18, 4) NULL,
	[BILLEDQTY] [decimal](18, 4) NULL,
	[BILLEDAMOUNT] [decimal](18, 4) NULL,
	[COMPLETED] [decimal](18, 4) NULL,
	[ORDBILLEDDIFF] [decimal](18, 4) NULL,
	[FINALIZED] [int] NULL,
 CONSTRAINT [PK__BUDGETWO__0E176F9AB314DE57] PRIMARY KEY CLUSTERED 
(
	[DETAILID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]