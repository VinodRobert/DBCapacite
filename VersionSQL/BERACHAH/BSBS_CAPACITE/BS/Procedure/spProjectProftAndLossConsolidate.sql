/****** Object:  Procedure [BS].[spProjectProftAndLossConsolidate]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
create PROCEDURE [BS].[spProjectProftAndLossConsolidate] (@ORGLISTS VARCHAR(800),@YEAR INT,@STARTPERIOD INT,@ENDPERIOD INT,@REPORTTYPE INT ,@IsConsoildate INT)
AS
BEGIN
...
SELECT PERIOD,ORGID,LEDGERCODE,
   ...
   CREDIT = CASE 
           ...
           END 
INTO #TRANSMASTER 
FROM TRANSACTIONS  T WHERE YEAR=@YEAR   AND PERIOD BETWEEN @STARTPERIOD AND @ENDPERIOD 
		...
		FROM 
		  (SELECT 
		   ...
		   FROM 
			#TRANSMASTER T 
		   WHERE 
			T.PERIOD<@STARTPERIOD AND T.PERIOD<>0 
		   GROUP BY 
		    T.ORGID,
			T.LEDGERCODE
	      ) O
...
END
 ...
 DELETE FROM #TEMP0  WHERE LEDGERCODE IN (@WIP,@RETAIN)
