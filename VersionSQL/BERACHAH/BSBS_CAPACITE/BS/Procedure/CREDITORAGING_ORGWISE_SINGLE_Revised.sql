/****** Object:  Procedure [BS].[CREDITORAGING_ORGWISE_SINGLE_Revised]    Committed by VersionSQL https://www.versionsql.com ******/

Create PROCEDURE [BS].[CREDITORAGING_ORGWISE_SINGLE_Revised](@YEAR INT,@ORGID INT,@CREDNO VARCHAR(10),@AGEDPERIOD INT,@FINYEAR INT)
AS
BEGIN
	...
	(
		SELECT 
			 ...
			 DATEDIFF(DAY,T.RECEIVEDDATE,@TRANSDATE) PERIOD,
			 0 AS           AGEBUCKET,
			 TRANSID 
		FROM
			TRANSACTIONS T 
		...
		WHERE 
			...
			YEAR >= @FINYEAR AND T.PERIOD=@AGEDPERIOD AND
			ORGID=@ORGID AND
			CREDNO = @CREDNO AND
			PAIDFOR <> 1 AND
			TRANSTYPE<>'CRA' 
      UNION
		SELECT 
			 ...
			 T.CREDNO       SUPPLIER,
			 C.CREDNAME     SUPPLIERNAME,
			 DATEDIFF(DAY,T.PDATE,@TRANSDATE) PERIOD,
			 0 AS           AGEBUCKET,
