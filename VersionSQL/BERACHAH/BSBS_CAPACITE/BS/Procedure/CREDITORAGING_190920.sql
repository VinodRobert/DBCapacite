/****** Object:  Procedure [BS].[CREDITORAGING_190920]    Committed by VersionSQL https://www.versionsql.com ******/

 

create PROCEDURE [BS].[CREDITORAGING_190920](@YEAR INT,@ZONE INT)
AS
BEGIN
	...
	UPDATE #TEMP0 SET BORGNAME = B.BORGNAME FROM BORGS  B INNER JOIN #TEMP0 ON #TEMP0.ORGID=B.BORGID 
	 ...
	 --- VALIDATION SECTION 
	 -- SELECT ORGID,SUM(CREDIT-DEBIT) AMOUNT 
	 --FROM TRANSACTIONS 
	 --WHERE LEDGERCODE='1315000'  AND YEAR>=2019 
	 --AND ORGID IN (SELECT BORGID FROM BORGS WHERE PARENTBORG=@ZONE)
	 --GROUP BY ORGID ORDER BY ORGID 
