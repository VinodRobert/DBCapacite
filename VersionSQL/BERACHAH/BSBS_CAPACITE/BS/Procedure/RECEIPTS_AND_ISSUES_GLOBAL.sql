/****** Object:  Procedure [BS].[RECEIPTS_AND_ISSUES_GLOBAL]    Committed by VersionSQL https://www.versionsql.com ******/

create PROCEDURE [BS].[RECEIPTS_AND_ISSUES_GLOBAL](@STARTPERIOD INT, @ENDPERIOD INT ,@FINYEAR INT) 
AS
...
SET @FILENAME = 'BSBS_TEMP.DBO.RECEIPTISSUE_GLOBAL'
...
SELECT STORECODE,STKCTL  INTO #TEMPSTORES  FROM INVSTORES  WHERE LEFT(STORECODE,4)='MS-9' ORDER BY STORECODE 
SELECT * FROM #TEMPSTORES

DECLARE STKBORG CURSOR FOR SELECT * FROM #TEMPSTORES

OPEN STKBORG
FETCH NEXT FROM STKBORG INTO @STORECODE,@STORELEDGERCODE 

WHILE @@FETCH_STATUS =0 
BEGIN
        
		SELECT @STOREBORG,@STORELEDGERCODE

		SELECT TOP 1 @STOREBORG=  BORGID FROM INVENTORY WHERE STKSTORE= @STORECODE 

		SELECT @STOREBORG,@STORELEDGERCODE,@STORECODE 


		INSERT INTO #MOVEMENT(BORGID,STKCTL,STORECODE,TRANSID,MATERIALCODE,RECEIPT,RECEIPTVALUE,TRANSTYPE) 
		SELECT @STOREBORG,@STORELEDGERCODE,@STORECODE,TRANSID,STOCKNO,QUANTITY,(DEBIT-CREDIT),TRANSTYPE FROM TRANSACTIONS WHERE STORE=@STORECODE AND YEAR=@FINYEAR AND PERIOD BETWEEN @STARTPERIOD AND @ENDPERIOD  AND 
