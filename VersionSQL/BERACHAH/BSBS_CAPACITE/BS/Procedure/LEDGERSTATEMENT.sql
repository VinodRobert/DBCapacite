/****** Object:  Procedure [BS].[LEDGERSTATEMENT]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [BS].[LEDGERSTATEMENT](@FINYEAR INT,@CREDNO VARCHAR(10),@STARTDATESTRING VARCHAR(15),@ENDDATESTRING VARCHAR(15) )
AS
BEGIN
	DECLARE @FILENAME VARCHAR(100)
	DECLARE @FILEID   VARCHAR(10)
	DECLARE @SQL      VARCHAR(250)
	SET @FILENAME = 'BSBS_TEMP.DBO.LEDGERSTATEMENT'
	DECLARE @BROUGHTFORWARD INT
	DECLARE @OBSTRING VARCHAR(50)
	...
	IF @BROUGHTFORWARD = 1 
	 BEGIN
	    SET @OBSTRING = 'OPENING BALANCE' 
	    SELECT @OPBAL_BILL= SUM(CREDIT-DEBIT) 
		FROM 
		 TRANSACTIONS 
		WHERE
		 CREDNO=@CREDNO AND YEAR=@FINYEAR AND PERIOD=0  AND TRANSTYPE IN ('DEL','CRI') AND LEDGERCODE = @CREDITORS 
		GROUP BY 
		 CREDNO 
		...
		FROM 
			TRANSACTIONS 
		WHERE
			CREDNO=@CREDNO AND YEAR=@FINYEAR AND PERIOD=0 AND TRANSTYPE NOT IN ('DEL','CRI') AND LEDGERCODE = @CREDITORS
		GROUP BY 
			CREDNO 
	...
	ELSE
	 BEGIN
	    SET @OBSTRING = 'BROUGHT FORWARD'
		SELECT @OPBAL_BILL= SUM(CREDIT-DEBIT) 
		FROM 
		 TRANSACTIONS 
		WHERE
		 CREDNO=@CREDNO AND YEAR=@FINYEAR AND PDATE < @STARTDATE AND TRANSTYPE IN ('DEL','CRI') AND LEDGERCODE = @CREDITORS 
		GROUP BY 
		 CREDNO 
	    ...
	    FROM 
	     TRANSACTIONS 
        WHERE
	     CREDNO= @CREDNO AND YEAR=@FINYEAR AND PDATE < @STARTDATE AND TRANSTYPE NOT IN ('DEL','CRI') AND LEDGERCODE = @CREDITORS
	   GROUP BY 
	     CREDNO 
     END
	...
	SELECT 
	  ORGID,RECEIVEDDATE BILLDATE,PDATE TRANSACTIONDATE,TRANSREF,LEFT(TRANSTYPE,3) TRANSTYPE,ORDERNO,CREDNO,DEBIT,CREDIT,TRANSID,TRANGRP,PERIOD 
	INTO 
	  #LEDGERSTATMENT
	FROM 
	  TRANSACTIONS 
    WHERE 
	  LEDGERCODE = @CREDITORS  AND  CREDNO = @CREDNO  AND PDATE >= @STARTDATE AND PDATE <= @ENDDATE AND YEAR >=@FINYEAR 
	ORDER BY 
	  ORGID 
	
	IF @BROUGHTFORWARD = 1
	   DELETE FROM #LEDGERSTATMENT WHERE PERIOD= 0
	...
	IF @OPENINGBALANCE <=0 
	   INSERT INTO #CLS
	   SELECT SPACE(10),SPACE(100),@CREDNO,SPACE(100),NULL,SPACE(10),SPACE(10),@OBSTRING, 0, ABS(@OPENINGBALANCE) , 0,0 
	 ELSE
	   INSERT INTO #CLS
	   SELECT SPACE(10),SPACE(100),@CREDNO,SPACE(100),NULL,SPACE(10),SPACE(10),@OBSTRING , 0, ABS(@OPENINGBALANCE) , 0 ,0 
    ...
    SET @SQL = 'SELECT *  INTO '+@FILENAME+ ' FROM   #CLS  ORDER BY ORGID,INDEXCODES  '
	EXEC(@SQL)
