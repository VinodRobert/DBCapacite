/****** Object:  Procedure [dbo].[sp_GetLookupData_Creditors]    Committed by VersionSQL https://www.versionsql.com ******/

-- =============================================
-- Author: Riaan Engelbrecht
-- Create date: 06-11-2020
-- Description:	Returns the creditors lookup data 
-- NOTES:
-- 2020-06-11 RiaanE
--  Added the procedure
-- 2020-06-30 RiaanE
-- Added attributes
-- =============================================
CREATE PROCEDURE [dbo].[sp_GetLookupData_Creditors] (
@PageNo INT = 1, 
...
BEGIN
  ...
  SET @Attrcols = STUFF((SELECT distinct ',' + QUOTENAME(c.ATTRIBUTE) 
   ...
   -- ======= End Attributes
	...
	SET @ExecSql = 'SELECT rtrim(CREDITORS.CREDNUMBER) crednumber, rtrim(CREDITORS.CREDNAME) credname ';
  if isnull(@Attrcols, '') <> ''
  BEGIN
		SET @ExecSql = @ExecSql + ',' + @Attrcols + ',[COLKEY] ';
  END;  
  
	SET @ExecSql = @ExecSql + 'FROM CREDITORS ';
  
  if isnull(@Attrcols, '') <> ''
  BEGIN
		SET @ExecSql = @ExecSql + 'LEFT OUTER JOIN (' + @AttrQuery + ') PRES on PRES.[COLKEY] = CREDITORS.CREDNUMBER ';
  END;
  
	SET @ExecSql = @ExecSql + 'where rtrim(ISNULL(CREDITORS.CREDNUMBER, '''')) <> '''' and CREDITORS.CREDSTATUS = 0 ';	
  ...
  --This is to establish the page numbers and record count
	...
	SET @ExecSql = @ResultSQL + ' ORDER BY ' + ISNULL(NULLIF(@SortString,''),'[CREDNUMBER]')
