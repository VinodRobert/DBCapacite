/****** Object:  Procedure [dbo].[sp_GetLookupData_Divisions]    Committed by VersionSQL https://www.versionsql.com ******/

-- =============================================
-- Author: Riaan Engelbrecht
-- Create date: 06-11-2020
-- Description:	Returns the division lookup data 
-- NOTES:
-- 2020-06-11 RiaanE
--  Added the procedure
-- 2020-06-30 RiaanE
-- Added attributes
-- =============================================
CREATE PROCEDURE [dbo].[sp_GetLookupData_Divisions] (
@PageNo INT = 1, 
...
BEGIN
  ...
  SET @Attrcols = STUFF((SELECT distinct ',' + QUOTENAME(c.ATTRIBUTE) 
   ...
   -- ======= End Attributes
	...
	SET @ExecSql = 'SELECT DIVISIONS.DIVID divid, DIVISIONS.DIVCODE divcode, DIVISIONS.DIVNAME divname, DIVISIONS.DIVSTATUS divstatus ';
  if isnull(@Attrcols, '') <> ''
  BEGIN
		SET @ExecSql = @ExecSql + ',' + @Attrcols + ',[COLKEY] ';
  END; 
	SET @ExecSql = @ExecSql + 'FROM DIVISIONS ';
  if isnull(@Attrcols, '') <> ''
  BEGIN
		SET @ExecSql = @ExecSql + 'LEFT OUTER JOIN (' + @AttrQuery + ') PRES on PRES.[COLKEY] = DIVISIONS.DIVID ';
  END;
  ...
  --This is to establish the page numbers and record count
	...
	SET @ExecSql = @ResultSQL + ' ORDER BY ' + ISNULL(NULLIF(@SortString,''),'[DIVID]')
