/****** Object:  Procedure [dbo].[sp_GetLookupData_Assets]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- Added attributes
-- =============================================
CREATE PROCEDURE [dbo].[sp_GetLookupData_Assets] (
@PageNo INT = 1, 
...
BEGIN
  ...
  SET @Attrcols = STUFF((SELECT distinct ',' + QUOTENAME(c.ATTRIBUTE) 
   ...
   -- ======= End Attributes
	...
	SET @ExecSql = 'SELECT AssetID assetid, ASSETNUMBER assetnumber, ASSETNAME assetname, ASSETSTATUS as assetstatus ';
  if isnull(@Attrcols, '') <> ''
  BEGIN
		SET @ExecSql = @ExecSql + ',' + @Attrcols + ',[COLKEY] ';
  END;
  
	SET @ExecSql = @ExecSql + 'FROM ASSETS ';
  
  if isnull(@Attrcols, '') <> ''
  BEGIN
		SET @ExecSql = @ExecSql + 'LEFT OUTER JOIN (' + @AttrQuery + ') PRES on PRES.[COLKEY] = ASSETS.ASSETNUMBER ';
  END;
  
	SET @ExecSql = @ExecSql + 'where ASSETS.ASSETSTATUS < 2 ';	
  ...
  --This is to establish the page numbers and record count
	...
	SET @ExecSql = @ResultSQL + ' ORDER BY ' + ISNULL(NULLIF(@SortString,''),'[ASSETNUMBER]')
