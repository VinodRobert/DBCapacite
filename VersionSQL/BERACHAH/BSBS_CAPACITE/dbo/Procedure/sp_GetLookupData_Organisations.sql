/****** Object:  Procedure [dbo].[sp_GetLookupData_Organisations]    Committed by VersionSQL https://www.versionsql.com ******/

-- =============================================
-- Author: Riaan Engelbrecht
-- Create date: 06-11-2020
-- Description:	Returns the organisation lookup data 
-- NOTES:
-- 2020-06-11 RiaanE
--  Added the procedure
-- =============================================
CREATE PROCEDURE [dbo].[sp_GetLookupData_Organisations] (
@PageNo INT = 1, 
...
BEGIN
  ...
  @TotalPages BIGINT = -1
	...
	SET @ExecSql = 'SELECT BORGID borgid, BORGNAME borgname
	' + 'FROM BORGS 
	' + 'where 1=1 ';	
  ...
  --This is to establish the page numbers and record count
	...
	SET @ExecSql = @ResultSQL + ' ORDER BY ' + ISNULL(NULLIF(@SortString,''),'[BORGID]')
