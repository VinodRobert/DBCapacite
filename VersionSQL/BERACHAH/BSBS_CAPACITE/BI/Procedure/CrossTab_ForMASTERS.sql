/****** Object:  Procedure [BI].[CrossTab_ForMASTERS]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE procedure [BI].[CrossTab_ForMASTERS] (@Select varchar(2000), 
...
set @sql = 'select ' + @GroupBy + @OtherCols + @Vals + ' from (' + @Select + ') Z GROUP BY ' + @GroupBy + ' Order by CREDNAME '
...
exec ( 'select ' + @GroupBy + @OtherCols + @Vals + 
       ' from (' + @Select + ') Z GROUP BY ' + @GroupBy + ' Order by CREDNAME '  )
