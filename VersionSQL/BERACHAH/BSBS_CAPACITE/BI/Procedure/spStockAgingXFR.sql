/****** Object:  Procedure [BI].[spStockAgingXFR]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE procedure [BI].[spStockAgingXFR](@CUTOFFDATE AS VARCHAR(15) , @LISTINVENTORY   LISTSTOCKITEMS READONLY , @LISTORGIDS  LISTORGIDS READONLY)
AS
...
BEGIN
		...
		SELECT  QUANTITY,PDATE,STOCKNO,ORGID,RATE,DEBIT 
		INTO #TRANSFERS
		FROM TRANSACTIONS 
		WHERE TRANSTYPE='XFR' AND QUANTITY>0 
		AND  ORGID IN (SELECT ORGID FROM #BORGS)

		ALTER TABLE #TRANSFERS ADD STKID INT
		UPDATE #TRANSFERS SET STKID = INV.STKID FROM INVENTORY INV INNER JOIN #TRANSFERS ON #TRANSFERS.OrgID = INV.BorgID AND #TRANSFERS.STOCKNO=INV.StkCode 
		...
		-- CONSIDERING THE XFR PLUS QTY AND ADDING TO DELIVERY
		INSERT INTO #TEMPDELIVERY
		SELECT QUANTITY,PDATE,0,0,ORGID,RATE,DEBIT,STKID,STOCKNO 
		FROM #TRANSFERS
