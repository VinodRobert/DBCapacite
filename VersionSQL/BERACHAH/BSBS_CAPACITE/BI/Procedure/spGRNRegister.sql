/****** Object:  Procedure [BI].[spGRNRegister]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE  PROCEDURE [BI].[spGRNRegister] (@STARTDATESTRING AS VARCHAR(15),@ENDDATESTRING AS VARCHAR(15), @LISTCREDITORS  LISTCREDNUMBERS  READONLY  ,@LISTORGIDS  LISTORGIDS READONLY ) 
AS
...
FROM DELIVERIES D 
WHERE
 D.TBORGID IN (SELECT ORGID FROM #BORGS)  AND 
...
DELETE FROM #TEMP0 WHERE CREDNO NOT IN (SELECT CREDNUMBER FROM #CREDITORS ) 
