/****** Object:  Procedure [BI].[spCashFlowMakers_NEW]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE   PROCEDURE  [BI].[spCashFlowMakers_NEW](@BORGID INT ,@FINYEAR INT ,@STARTPERIOD INT,@ENDPERIOD INT)
AS
...
INSERT INTO #BORGS VALUES (10) 
--SELECT ORGID  FROM @BORGS
DELETE FROM #BORGS WHERE ORGID IS NULL



CREATE TABLE #FINALOUTPUT(ID INT PRIMARY KEY IDENTITY(1,1), ORGID INT, FLOWTYPE INT, ITEMNAME VARCHAR(60), AMOUNT DECIMAL(18,2) )
DELETE FROM #FINALOUTPUT 
INSERT INTO #FINALOUTPUT(ORGID,FLOWTYPE,ITEMNAME,AMOUNT) 
SELECT @BORGID,FLOWTYPE,ITEMNAME,0 FROM BI.CASHFLOWTEMPLATE
...
BEGIN 
			...
			SET @TEMPSUM = 0
			SELECT @TEMPSUM = SUM(COL1VALUE) FROM BS.CASHFLOW_NEW WHERE COL1INDEX IN ( '1000', '1005', '1010', '1020') 
			...
			SET @TEMPSUM = 0
			SELECT @TEMPSUM = SUM(COL1VALUE) FROM BS.CASHFLOW_NEW WHERE COL1INDEX IN ( '1120', '1130', '1160' , '1170', '1180')  
      ...
      UPDATE #FINALOUTPUT SET AMOUNT = CF.COL1VALUE FROM BS.CASHFLOW_NEW CF INNER JOIN #FINALOUTPUT ON #FINALOUTPUT.ITEMNAME =  CF.COL1NAME WHERE #FINALOUTPUT.FLOWTYPE=1
	  UPDATE #FINALOUTPUT SET AMOUNT = CF.COL2VALUE FROM BS.CASHFLOW_NEW CF INNER JOIN #FINALOUTPUT ON #FINALOUTPUT.ITEMNAME =  CF.COL2NAME WHERE #FINALOUTPUT.FLOWTYPE=2
	  UPDATE #FINALOUTPUT SET AMOUNT = CF.COL1VALUE FROM BS.CASHFLOW_NEW CF INNER JOIN #FINALOUTPUT ON #FINALOUTPUT.ITEMNAME =  CF.COL1NAME WHERE #FINALOUTPUT.FLOWTYPE=3
