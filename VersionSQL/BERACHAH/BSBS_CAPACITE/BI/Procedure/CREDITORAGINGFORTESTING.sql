/****** Object:  Procedure [BI].[CREDITORAGINGFORTESTING]    Committed by VersionSQL https://www.versionsql.com ******/

 

CREATE PROCEDURE [BI].[CREDITORAGINGFORTESTING](@AGEDTO INT,  @AGEDTOPERIOD INT, @FINYEAR INT, @AGEOF INT )  
AS
BEGIN
 ...
 SELECT COUNT(*) FROM #DUMPS 
	...
	SELECT CREDNO, PDATE,(DEBIT-CREDIT) PAYMENTAMOUNT
	INTO #UNMATCHEDTRANSACTIONS 
	FROM #DUMPS 
	WHERE TRANSTYPE IN ('CBC','CBD' , 'PCP','PCR')

	SELECT * FROM #UNMATCHEDTRANSACTIONS   WHERE PAYMENTAMOUNT>0 
	...
	---- MODIFICATION DONE ON 05 MARCH 2019 -
	---- WE ARE TRYING TO KNOCK OF UNMATCHED WITH OUTSTANDING 

	--CREATE TABLE #UMATCHEDHISTORY(INDEXCODE  INT PRIMARY KEY IDENTITY(1,1),SUPPLIERID VARCHAR(15), PDATE DATETIME,AMOUNT DECIMAL(18,2))
	--DECLARE @MATCHINGSUPPLIER VARCHAR(15)
	--DECLARE @UNMATCHEDAMOUNT DECIMAL(18,2) 
	--DECLARE @CELLVALUE DECIMAL(18,2)
	--DECLARE @LINENO INT 
	--DECLARE @MAXLINES INT
	--DECLARE @LINEPAYMENTAMOUNT DECIMAL(18,2)
	--DECLARE @BALANCEAMOUNT DECIMAL(18,2) 

	--DECLARE MATCHINGUNMATCHED CURSOR FOR 
	--SELECT SUPPLIERID,UNMATCHED FROM #FINALCONSOLIDATEDORGLEVEL WHERE UNMATCHED<0 AND SUPPTOTAL>0 
	--OPEN MATCHINGUNMATCHED 
	--FETCH NEXT FROM MATCHINGUNMATCHED INTO @MATCHINGSUPPLIER,@UNMATCHEDAMOUNT 
	--WHILE @@FETCH_STATUS = 0 
	--BEGIN
	--   DELETE FROM #UMATCHEDHISTORY
	--   INSERT INTO #UMATCHEDHISTORY
	--   SELECT CREDNO,PDATE,PAYMENTAMOUNT  
	--   FROM #UNMATCHEDTRANSACTIONS
	--   WHERE CREDNO=@MATCHINGSUPPLIER 
	--   ORDER BY PDATE ASC 
	--   SELECT @MAXLINES = COUNT(*) FROM #UNMATCHEDTRANSACTIONS
	--   SET @LINENO = 1
	   
	--   SELECT @CELLVALUE  = BUCKET7 FROM #FINALCONSOLIDATEDORGLEVEL WHERE SUPPLIERID=@MATCHINGSUPPLIER
	--   IF @CELLVALUE >0 
	--    BEGIN
	--	  SET @BALANCEAMOUNT = @CELLVALUE 
	--	  WHILE (@LINENO <= @MAXLINES) AND  (@BALANCEAMOUNT>0)
	--	  BEGIN 
	--	     SELECT @LINEPAYMENTAMOUNT = PAYMENTAMOUNT FROM #UMATCHEDHISTORY WHERE INDEXCODE=@LINENO 
	--		 IF @CELLVALUE > @LINEPAYMENTAMOUNT 
	--		    BEGIN
	--			   SET @BALANCEAMOUNT = @CELLVALUE - @LINEPAYMENTAMOUNT 
	--			   DELETE FROM 
	--			END 
	--	  END 
	...
	--END 
	--CLOSE MATCHINGUNMATCHED
	--DEALLOCATE MATCHINGUNMATCHED
 ...
 --   SELECT * FROM #FINALCONSOLIDATED         ORDER BY SUPPLIERID,ORGID  
    SELECT * FROM #FINALCONSOLIDATED WHERE SUPPLIERID='27AKHPJ355' 
